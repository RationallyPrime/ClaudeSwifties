...
# No jq means no ingress, but the status line still has to render.
if ! command -v jq >/dev/null 2>&1; then
    printf '%s' "$input" | sed -n 's/.*"display_name":"\([^"]*\)".*/[\1]/p'
    exit 0
fi

# Bail before anything else if stdin isn't parseable, so a malformed payload
# renders a plain status line instead of jq errors and an empty "[]".
...
# --- status line output (this is what you actually see) ---------------------
line="[$model]"
